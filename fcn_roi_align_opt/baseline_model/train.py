...
def train_model(model, optimizer, scheduler, dataloaders, dataset_sizes, class_weights, track_sample, batch_size = 32, num_epochs=25):
    since = time.time()
    PATH = 'save_objs_conv.pth'
    epo = 1
        ...
        for phase in ['train']:
            ...
            for mini_batch, (inputs, labels) in enumerate(dataloaders[phase]):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    loss = F.nll_loss(outputs, labels, weight=class_weights) + dice_loss(input= labels, target=preds)
