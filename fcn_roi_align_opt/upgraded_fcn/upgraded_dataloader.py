...
ROOT_DIR = Path.cwd().parents[0].parents[0]
...
class MultiScales(object):
    ...
    def call(self, sample):
        ...
        for scale in self.scales:
            rescaled = transform.resize(label, (self.H/scale, self.W/scale))
            multi_scales.append(rescaled)

        return sample, multi_scales
...
class ToTupleTensorMultipleScales(object):
    ...
    def __call__(self, sample):
        ...
        for i in range(len(multi_scales)):
            multi_scales[i] = torch.from_numpy(multi_scales[i])

        return scans, label, multi_scales
