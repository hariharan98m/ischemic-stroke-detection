            ...
            for phase in ['train', 'val']:
                ...
                for mini_batch, (inputs, label224, label28) in enumerate(dataloaders[phase]):
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        outputs28 = torch.exp(outputs28)

                        loss = F.nll_loss(outputs28, mask28.squeeze(), weight=class_weights)

                        torch_pred224_prob = get_prob_map224(outputs28)
                        # return format is (batch_size, 1, 224, 224)

                        dice_l = dice_loss(input=torch_pred224_prob, target=label224).item()

                        # dice_l = dice_loss(input=outputs28, target=mask28)

                        total_loss = 0.7* dice_l + 0.3*loss
