...
class FCN8s(nn.Module):
    ...
    def __init__(self, pretrained_net, n_class=2):
        ...
        self.last = nn.Conv2d(256, 1, kernel_size=1, stride=1, padding=0)

        self.cls = nn.Conv2d(1, 1, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        in_channels_classes = self.relu(self.fcn_conv_from_stacked1(score))
        in_channels_classes = self.bn512(in_channels_classes)
        # 512 to 256 channels
        in_channels_classes = self.relu(self.fcn_conv_from_stacked2(in_channels_classes))
        in_channels_classes = self.bn256(in_channels_classes)
        # 256 to n_class channels
        out_classes = self.relu(self.last(in_channels_classes))

        # final classification conv layer
        cls_layer = self.sigmoid(self.cls(out_classes))
