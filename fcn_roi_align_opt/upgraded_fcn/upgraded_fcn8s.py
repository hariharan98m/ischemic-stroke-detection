...
class UpgradedFCN8s(nn.Module):
    ...
    def __init__(self, pretrained_net, n_class=2):
        ...
        self.block1_ext_conv = nn.Conv2d(64, n_class, kernel_size=3, padding=1)
        self.block1_bn = nn.BatchNorm2d(n_class)

        self.block2_ext_conv = nn.Conv2d(128, n_class, kernel_size=3, padding=1)
        self.block2_bn = nn.BatchNorm2d(n_class)

        self.block3_ext_conv = nn.Conv2d(256, n_class, kernel_size=3, padding=1)
        self.block3_bn = nn.BatchNorm2d(n_class)
        ...
        self.deconv_fcn_conv1 = nn.ConvTranspose2d(4096, 4096, kernel_size=2, stride=5, padding=2, dilation=1,
                                                   output_padding=0)
    ...
    def forward(self, x):
        ...
        pool1 = output['x1']  # size = (N, 64, H/2, W/2)
        pool2 = output['x2']  # size= (N, 128, x.H/4,  x.W/4)
        ...
        block1 = F.log_softmax(self.block1_ext_conv(pool1), dim=1)
        block2 = F.log_softmax(self.block2_ext_conv(pool2), dim=1)
        block3 = F.log_softmax(self.block3_ext_conv(pool3), dim=1)

        return {
            'block1': block1,
            'block2': block2,
            'block3': block3,
            'mask': log_softmax_activated_scores
        }
